...
| **Nominal Response Model (Bock, 1972)** | **Primary for the nominal question**, fitted in `invariance/nrm.py`. Dichotomising to correct/incorrect throws away *which* wrong answer was given. The NRM models all six response options as an unordered set — `P(k | theta) = softmax(a_k theta + c_k)` — which is the only family here that does not require the responses to be ordered. DIF is IRT-LR-DIF (Thissen, Steinberg & Wainer): anchor the other five items, free the studied item across groups, test. "Group A hears fear as sadness, group B hears it as fear" is invisible to LR-DIF and visible here. Fitted by marginal-ML EM; cross-checked against R `mirt` (`invariance/mirt_check.R`). |
| **Matched-decile response-share comparison** | **Approximation, kept and labelled.** `nominal_shift` in `dif.py`. Was the stand-in for the NRM before the NRM was fitted. Retained so the cheap approximation and the fitted model can be compared; reported as `decile_tvd_approx`, never as an NRM result. |
